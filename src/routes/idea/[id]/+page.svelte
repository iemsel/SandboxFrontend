<script>
  ...
  function gotoLogin() {
  goto(
    resolveRoute(`/login?returnUrl=${encodeURIComponent($page.url.pathname)}`)
  );
}
...
<div class="flex min-h-screen">
  ...
  <div class="flex-1 transition-all duration-300 {showAI ? 'mr-[480px]' : 'mr-0'}">
    <div class="w-full px-4 py-8 max-w-7xl mx-auto">
      <!-- Back Button -->
      <a href={resolveRoute('/')} class="mb-6 inline-flex items-center gap-2">
      ...
      <div class="grid gap-8 lg:grid-cols-[1fr_400px] w-full">
        ...
        <div class="space-y-8">
          ...
          <div class="flex flex-wrap gap-3">
            {#if $authStore.isLoggedIn}
              <a
                href={resolveRoute(`/planner/${idea.id}`)}
            ...
            <button
              ...
              class="px-4 py-2 rounded border border-[var(--color-border)] cursor-pointer font-medium hover:bg-[var(--color-border-light)] transition-colors"
            >
              Print
            </button>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="border border-[var(--color-border)] rounded-lg p-6 bg-white">
              <ul class="space-y-3">
                {#each idea.tools as tool (tool)}
                  <li class="flex items-start gap-3">
                    <span
                      class="mt-1 h-5 w-5 rounded-full border-2 border-[var(--color-primary)] bg-white"
                    ></span>
          ...
          <div class="space-y-4">
            ...
            <div class="border border-[var(--color-border)] rounded-lg p-6 bg-white">
              <ol class="space-y-6">
                {#each idea.instructions as step, i (i)}
                  <li class="flex gap-4">
                    <span
                      class="h-8 w-8 flex items-center justify-center rounded-full bg-[var(--color-primary)] text-white font-bold flex-shrink-0"
                    >
                      {i + 1}
                    </span>
                  ...
                  <div class="space-y-2">
                    ...
                    <div class="flex gap-1" role="group" aria-label="Rating">
                      {#each [1, 2, 3, 4, 5] as rating (rating)}
                        ...
                        >
                          <span
                            class="text-2xl {rating <= (newCommentRating || 0)
                              ? 'text-yellow-400'
                              : 'text-gray-300'}"
                          >
                            ★
            ...
            {#each comments as c (c.id)}
              <div class="border border-[var(--color-border)] rounded-lg p-6 bg-white">
                <div class="flex gap-4">
                  <div
                    class="h-10 w-10 rounded-full bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-light)] flex items-center justify-center font-bold text-white"
                  >
                    {c.avatar}
                  </div>
                  <div class="flex-1">
                    <div class="mb-1 flex items-center gap-2">
                      ...
                      <div class="flex gap-0.5">
                        {#each Array(c.rating) as _, index (index)}
                          <span class="text-yellow-400">★</span>
                        {/each}
                        {#each Array(5 - c.rating) as _, index (index)}
                    ...
                    <div class="flex items-center gap-4 text-sm">
                      <button
                        class="flex items-center gap-1 px-2 py-1 rounded transition-colors font-medium {c.userReaction ===
                        'like'
        ...
        <div class="space-y-6">
          <div
            class="border border-[var(--color-border)] rounded-lg overflow-hidden bg-white relative"
          >
            <img src={idea.imageUrl} alt={idea.title} class="w-full h-96 object-cover" />
            <div class="absolute right-3 top-3 print:hidden">
              <button
                class="rounded-full bg-white/80 backdrop-blur-sm p-2 transition-colors hover:bg-white shadow-md hover:shadow-lg {isFavorited
                  ? 'text-red-500'
                  : 'text-gray-600'}"
          ...
          <div
            class="border border-[var(--color-border)] rounded-lg p-6 bg-white space-y-6 ratings-panel"
          >
              ...
              <div class="space-y-3">
                <div class="flex items-center gap-3">
                  <div class="flex gap-1">
                    {#each Array(Math.round(idea.avgRating)) as _, i (i)}
                      <span class="text-3xl text-yellow-400">★</span>
                    {/each}
                    {#each Array(5 - Math.round(idea.avgRating)) as _, i (i)}
                  ...
                  <div class="flex flex-col">
                    ...
                    <span class="text-sm text-gray-600">
                      {idea.ratingCount}
                      {idea.ratingCount === 1 ? 'rating' : 'ratings'}
                    </span>
                  </div>
                </div>
              </div>
  ...
  {#if showAI}
    ...
    >
      <!-- Header -->
      <div
        class="flex items-center justify-between p-6 border-b border-[var(--color-border)] bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-light)]"
      >
        <div class="flex items-center gap-3">
          <div
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-4">
        <div class="space-y-3">
          <button
            class="w-full p-4 rounded-xl text-white bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-light)] text-left hover:from-[var(--color-text-primary)] hover:to-[var(--color-primary)] transition-all duration-200 shadow-sm hover:shadow-md"
          >
        ...
        <div class="pt-6 border-t border-[var(--color-border)] sticky top-[100vh]">
          <div class="relative">
            ...
            <button
              class="absolute right-4 top-4 h-8 w-8 rounded-lg bg-[var(--color-primary)] flex items-center justify-center hover:bg-[var(--color-primary)] transition-colors"
            >
