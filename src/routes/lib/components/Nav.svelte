...
>
  <div class="flex items-center gap-2 text-xl font-semibold">
    ...
    <a
      href="/help"
      class="bg-[var(--color-text-secondary)] text-[var(--color-bg)] font-bold rounded-full w-6 h-6 flex items-center justify-center"
      >?</a
    >
  </div>
...
{#if showDropdown}
  <div class="fixed inset-0 z-[50]" on:click={() => (showDropdown = false)}></div>
{/if}
